...
interface ChapterBodyDao {
    ...
    @Query("SELECT COUNT(*) FROM ChapterBody WHERE url IN (SELECT url FROM Chapter WHERE bookUrl = :bookUrl)")
    suspend fun getDownloadedChapterCount(bookUrl: String): Int
}